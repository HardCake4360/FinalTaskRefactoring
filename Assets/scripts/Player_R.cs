...
{
    void Execute(Player_R player);
}

//커맨드 구현
public class JumpCommand : ICommand
{
    public void Execute(Player_R player)
    {
...
{
    private Dictionary<KeyCode, ICommand> keyDownCommands = new();

    public void BindCommand(KeyCode key, ICommand command)
    {
        keyDownCommands[key] = command;
    }

    public void HandleInput(Player_R player)
    {
        foreach (var entry in keyDownCommands)
        {
            if (Input.GetKeyDown(entry.Key))
            {
                entry.Value.Execute(player);
            }
        }
    }
}
...
public class Player_R : MonoBehaviour
{
    ...
    private static Player_R player;
    ...
    void Start()
    {
        ...
        inputHandler.BindCommand(KeyCode.Space, new JumpCommand());
        inputHandler.BindCommand(KeyCode.Mouse0, new AttackCommand());
    ...
    void UpdateEquipmentStatus()
    {
        ...
        shoes.SetActive(shoed);
        wings.SetActive(winged);
    }
